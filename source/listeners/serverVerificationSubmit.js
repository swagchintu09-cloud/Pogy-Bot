...
module.exports = class serverVerificationSubmit extends Event {
    ...
    async run(interaction) {
        ...
        if (og_verification_code !== value_verification_code) {
            return await interaction?.editReply({ content: "RETRY! Verification code is incorrect." });
        };
        let data = await this.client.database.guildVerificationData.get(interaction?.guild.id);
        if (!data) {
            return await interaction?.editReply({ content: "Server verification data not found!" });
        };
        if (!data.enabled) {
            return await interaction?.editReply({ content: "Server Verification is disabled contact server admins to get verified" });
        };
        ...
        if (role_to_give == null)  {
            return await interaction?.editReply({ content: "Server verification role not found!" });
        };
        await interaction?.member.roles.add(role_to_give, `Server Verification`);
        return await interaction?.editReply({ content: "You have been verified!" });
    }
