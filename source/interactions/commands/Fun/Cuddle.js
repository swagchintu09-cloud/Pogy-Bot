...
module.exports = class Cuddle extends Command {
    ...
    async render(actor, guild, rawUser, responder) {
        ...
        const mediaUrl = await safeAction(() => nekoClient.sfw.cuddle());
        const panel = buildPanel({
            title: "Cuddle",
            lines: [
                "**" + actor.username + "** cuddles **" + member.user.username + "**.",
    ...
    async run({ message, args }) {
        ...
        if (!lookup) {
            return message?.reply("Please provide a user to cuddle!");
        }
    ...
    async exec({ interaction }) {
        ...
        if (!user) {
            return interaction?.reply("Please provide a user to cuddle!");
        }
