...
module.exports = class Kill extends Command {
    ...
    async render(actor, guild, rawUser, responder) {
        ...
        const panel = buildPanel({
            title: "Kill",
            lines: [
                "**" + actor.username + "** absolutely ended **" + member.user.username + "**.",
    ...
    async run({ message, args }) {
        ...
        if (!lookup) {
            return message?.reply("Please provide a user to kill!");
        }
    ...
    async exec({ interaction }) {
        ...
        if (!user) {
            return interaction?.reply("Please provide a user to kill!");
        }
