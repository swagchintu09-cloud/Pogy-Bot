...
module.exports = class Feed extends Command {
    ...
    async render(actor, guild, rawUser, responder) {
        ...
        if (member.id === actor.id) {
            return responder.reply("You can't feed yourself!");
        }
        if (member.id === this.client.user.id) {
            return responder.reply("You can't feed me!");
        }
        const mediaUrl = await safeAction(() => nekoClient.sfw.feed());
        const panel = buildPanel({
            title: "Feed",
            lines: [
                "**" + actor.username + "** feeds **" + member.user.username + "**.",
    ...
    async run({ message, args }) {
        ...
        if (!lookup) {
            return message?.reply("Please provide a user to feed!");
        }
    ...
    async exec({ interaction }) {
        ...
        if (!user) {
            return interaction?.reply("Please provide a user to feed!");
        }
