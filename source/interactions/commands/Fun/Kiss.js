...
module.exports = class Kiss extends Command {
    ...
    async render(actor, guild, rawUser, responder) {
        ...
        const mediaUrl = await safeAction(() => nekoClient.sfw.kiss());
        const panel = buildPanel({
            title: "Kiss",
            lines: [
                "**" + actor.username + "** kisses **" + member.user.username + "**.",
    ...
    async run({ message, args }) {
        ...
        if (!lookup) {
            return message?.reply("Please provide a user to kiss!");
        }
    ...
    async exec({ interaction }) {
        ...
        if (!user) {
            return interaction?.reply("Please provide a user to kiss!");
        }
