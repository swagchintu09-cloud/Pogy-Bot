...
const Giveaway = require("./giveaway-session");
...
module.exports = class GiveawayManager {
  ...
  async mainEmbed(giveaway) {
    if (giveaway.messages.title)
      giveaway.messages.title = giveaway.messages.title.replace(
        "{prize}",
        giveaway.prize
      );
    if (giveaway.messages.description) {
      giveaway.messages.description = giveaway.messages.description.replace(
        "{prize}",
        giveaway.prize
      );
      giveaway.messages.description = giveaway.messages.description.replace(
      ...
      giveaway.messages.description = giveaway.messages.description.replace(
        "{timestamp}",
        `${this.client.util.discordFormatTime(
          giveaway.endsAt,
          "R"
        )} ${this.client.util.discordFormatTime(giveaway.endsAt, "T")}`
      );
      giveaway.messages.description = giveaway.messages.description.replace(
        "{entries}",
        giveaway.entered.length
      );
      giveaway.messages.description = giveaway.messages.description.replace(
        "{hoster}",
        giveaway.hoster
      ...
      giveaway.messages.description = giveaway.messages.description.replace(
        "{winners}",
        giveaway.winners.join(", ").length == 0
          ? "None"
    ...
    const embed = this.client.util
      ...
      .setDescription(
        giveaway.messages.description ||
          `Ends at ${this.client.util.discordFormatTime(
            giveaway.endsAt,
            "R"
          )} ${this.client.util.discordFormatTime(
            giveaway.endsAt,
      ...
      .setColor(
        Number.isInteger(giveaway?.messages?.embedColor)
          ? giveaway.messages.embedColor
          : 0x2b2d31
      )
    ...
    if (giveaway.endsAt !== Infinity) {
      ...
      embed.setFooter({
        text: `${giveaway.winnerCount} winner${
  ...
  async endEmbed(giveaway) {
    ...
    const embed = this.client.util
      ...
      .setDescription(
        giveaway.messages.description ||
          `Ended ${this.client.util.discordFormatTime(
            giveaway.endsAt,
            "R"
          )} ${this.client.util.discordFormatTime(giveaway.endsAt, "T")}\n${
  ...
  async start(channel, options) {
    return new Promise(async (resolve, reject) => {
      ...
      const hosterId =
        typeof options?.hoster === "string"
          ? options.hoster
          : options?.hoster?.id;
      if (!hosterId) {
        return reject("giveaway hoster is missing or invalid.");
      }
      let giveaway = new Giveaway(this, {
        ...
        reaction: options.reaction || this.client.config.Client.emoji.giveaway2,
      });
  ...
  async updateStaticGiveaways() {
    ...
    giveaways.forEach(async (giveaway) => {
      let channel = await this.client.channels.fetch(giveaway.channelId).catch(() => null);
      if (!channel) return;
