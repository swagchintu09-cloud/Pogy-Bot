module.exports = class SetupFunction {
  ...
  async setup(message, slash = false) {
    ...
    let msg = await message?.channel.send(
      `${this.client.config.Client.emoji.loading} **Starting Setup**`
    );
    let guildData = await this.client.database.guildData.get(message?.guild.id);
    let guildVerificationData =
      await this.client.database.guildVerificationData.get(message?.guild.id);
    await this.client.sleep(2000);
    let embed = this.client.util
      .embed()
      .setTitle(`Welcome to the PogyClient setup`)
      .setThumbnail(hemuuser.displayAvatarURL())
      .setDescription(
        `${this.client.config.Client.emoji.info} This command will help you setup the bot for your server. You can always run this command again to change your settings.`
      )
      .setColor(this.client.config.Client.PrimaryColor)
      .setFooter({ text: `PogyClient Setup`, iconURL: message.guild.iconURL() });
    await msg.edit({ content: ``, embeds: [embed] });
    await this.client.sleep(5000);
    embed.setDescription(
      `Server Prefix is currently set to \`${guildData.prefix}\` Would you like to change it?`
    );
    ...
    await new Promise((resolve) => {
      collector.on("collect", async (i) => {
        if (i.customId === "yes") {
          await i.deferUpdate();
          nunumsg = await message?.channel.send(
            `${this.client.config.Client.emoji.loading} **Please type the new prefix you would like to set.**`
          );
          let filter2 = (m) => {
          ...
          collector2.on("collect", async (m) => {
            if (m.content.length > 5)
              return nunumsg.edit(
                `${this.client.config.Client.emoji.cross} **Prefix can't be longer than 5 characters.**`
              );
            if (m.content.length < 1)
              return nunumsg.edit(
                `${this.client.config.Client.emoji.cross} **Prefix can't be shorter than 1 character.**`
              );
            guildData.prefix = m.content;
            await this.client.database.guildData.putPrefix(
              message?.guild.id,
              guildData
            );
            await nunumsg.edit(
              `${this.client.config.Client.emoji.tick} **Prefix has been set to \`${m.content}\`**`
            );
            await m.delete();
          ...
          collector2.on("end", async (collected, reason) => {
            if (reason === "time") {
              await nunumsg.edit(
                `${this.client.config.Client.emoji.cross} **You took too long to respond.**`
              );
              await collector2.stop();
            }
          });
    ...
    if (!check.some((r) => r.name.includes("PogyClient"))) {
      embed.setDescription(
        `${this.client.config.Client.emoji.info} Auto Moderation is currently **Disabled**. Would you like to change it?`
      );
      compos = [
        {
          type: 1,
      ...
      await new Promise((resolve) => {
        collector3.on("collect", async (i) => {
          if (i.customId === "enable") {
            await i.deferUpdate();
            nunumsg = await message?.channel.send(
              `${this.client.config.Client.emoji.loading} Creating Auto Moderation Rules...`
            );
            if (check.size > 0) {
              for (const rule of check) {
                await message?.guild.autoModerationRules
                  .delete(rule[0])
                  .catch((err) => {});
              }
            }
            ...
            await message?.guild.autoModerationRules
              .create({
                name: "PogyClient AutoMod Rule 1",
                eventType: 1,
                triggerType: 1,
                triggerMetadata: {
            ...
            await message?.guild.autoModerationRules
              .create({
                name: "PogyClient AutoMod Rule 2",
                eventType: 1,
                triggerType: 5,
                triggerMetadata: {
                  mentionTotalLimit: 5,
            ...
            await message?.guild.autoModerationRules
              .create({
                name: "PogyClient AutoMod Rule 3",
                eventType: 1,
                triggerType: 4,
                triggerMetadata: {
                  presets: [1, 2, 3],
            ...
            await message?.guild.autoModerationRules
              .create({
                name: "PogyClient AutoMod Rule 4",
                eventType: 1,
                triggerType: 3,
                actions: [
                  {
            ...
            await nunumsg.edit(
              `${this.client.config.Client.emoji.tick} Auto-moderation has been enabled!`
            );
            automodon = true;
      ...
      collector3.on("end", async (collected, reason) => {
        if (reason === "time") {
          await nunumsg.edit(
            `${this.client.config.Client.emoji.cross} Command timed out!`
          );
          await collector3.stop();
          await msg.delete();
          return;
    ...
    if (guildData.welcome.channel == null) {
      embed.setDescription(
        `${this.client.config.Client.emoji.info} Welcome Is Disabled!, Would You Like To Enable It?`
      );
      compos = [
        {
          type: 1,
      ...
      await new Promise((resolve) => {
        collector4.on("collect", async (i) => {
          if (i.customId === "yes") {
            await i.deferUpdate();
            nunumsg = await message?.channel.send(
              `${this.client.config.Client.emoji.channel} Please mention the channel you want to set as the welcome channel!`
            );
            let filter5 = (m) => {
              if (m.author.id === message?.member.id) return true;
              return false;
            };
            const collector5 = message?.channel.createMessageCollector({
              filter: filter5,
              time: 60000,
            });
...
Need more presets? Try \`/embed create\`!`,
              ...
              const collector6 = nunumsg.createMessageComponentCollector({
                filter: filter6,
                time: 60000,
              });
              let presetName = "";
    ...
    if (!guildData.greet.enabled) {
      embed.setDescription(
        `${this.client.config.Client.emoji.info} Auto delete greet message is disabled!, Would You Like To Enable It?`
      );
      compos = [
        {
          type: 1,
      ...
      const collector7 = msg.createMessageComponentCollector({
        filter: filter7,
        time: 60000,
      });
      await new Promise((resolve) => {
        collector7.on("collect", async (i) => {
          if (i.customId === "yes") {
            ...
            const collector8 = message?.channel.createMessageCollector({
              filter: filter8,
              time: 60000,
            });
            collector8.on("collect", async (m) => {
              ...
              await this.client.database.guildData.putGreet(
                message?.guild.id,
                guildData.greet
              );
              await nunumsg.edit(
                `${this.client.config.Client.emoji.tick} Auto delete greet message has been enabled!`
              );
    ...
    if (!guildData.autorole.enabled) {
      embed.setDescription(
        `${this.client.config.Client.emoji.info} Auto role is disabled!, Would You Like To Enable It?`
      );
      compos = [
        {
          type: 1,
      ...
      const collector9 = msg.createMessageComponentCollector({
        filter: filter9,
        time: 60000,
      });
      await new Promise((resolve) => {
        collector9.on("collect", async (i) => {
          if (i.customId === "yes") {
            await i.deferUpdate();
            nunumsg = await message?.channel.send(
              `${this.client.config.Client.emoji.add} Please mention the role you want to set as the auto role!`
            );
            let filter10 = (m) => {
              if (m.author.id === message?.member.id) return true;
              return false;
            };
            const collector10 = message?.channel.createMessageCollector({
              filter: filter10,
              time: 60000,
            });
            collector10.on("collect", async (m) => {
              ...
              await this.client.database.guildData.putAutorole(
                message?.guild.id,
                guildData.autorole
              );
              await nunumsg.edit(
                `${this.client.config.Client.emoji.tick} Auto role has been enabled!`
              );
    ...
    if (!guildVerificationData.enabled) {
      embed.setDescription(
        `${this.client.config.Client.emoji.info} Verification is disabled!, Would You Like To Enable It?`
      );
      compos = [
        {
          type: 1,
      ...
      const collector11 = msg.createMessageComponentCollector({
        filter: filter11,
        time: 60000,
      });
      ...
      await new Promise((resolve) => {
        collector11.on("collect", async (i) => {
          if (i.customId === "yes") {
            ...
            nunumsg = await message?.channel.send(
              `${this.client.config.Client.emoji.channel} Please mention the channel you want to set as the verification channel!`
            );
            let filter12 = (m) => {
              if (m.author.id === message?.member.id) return true;
              return false;
            };
            const collector12 = message?.channel.createMessageCollector({
              filter: filter12,
              time: 60000,
            });
            collector12.on("collect", async (m) => {
              ...
              isargtaken = true;
              nunumsg.edit(
                `${this.client.config.Client.emoji.add} Please mention the role you want to set as the verification role!`
              );
              let filter13 = (m) => {
                if (m.author.id === message?.member.id) return true;
                return false;
              };
              const collector13 = message?.channel.createMessageCollector({
                filter: filter13,
                time: 60000,
              });
              collector13.on("collect", async (m) => {
                ...
                await this.client.database.guildVerificationData.post(
                  message?.guild.id,
                  guildVerificationData
                );
                await nunumsg.edit(
                  `${this.client.config.Client.emoji.tick} Verification has been enabled!`
                );
                let veriembed = this.client.util
                  .embed()
                  .setDescription(
                    `Welcome to ${message?.guild.name}! Please react to this message to verify yourself.`
                  )
                  .setColor(this.client.config.Client.PrimaryColor)
                  .setFooter({
                    text: `© ${message?.guild.name} | Verification`,
    ...
    if (!guildData.presenserole.enabled) {
      embed.setDescription(
        `${this.client.config.Client.emoji.info} Presence Role is disabled!, Would You Like To Enable It?`
      );
      compos = [
        {
          type: 1,
      ...
      const collector14 = msg.createMessageComponentCollector({
        filter: filter14,
        time: 60000,
      });
      // for verification we have to collect 2 things first the channel and then the role so we will use a promise
      await new Promise((resolve) => {
        collector14.on("collect", async (i) => {
          if (i.customId === "yes") {
            ...
            nunumsg = await message?.channel.send({
              content: `${this.client.config.Client.emoji.add} Please select the game you want to set as the game for the presence role!`,
            ...
            const collector15 = nunumsg.createMessageComponentCollector({
              filter: filter15,
              time: 60000,
            });
            collector15.on("collect", async (i) => {
              ...
              if (i.customId === "selectmenu") {
                await nunumsg.edit(
                  `${this.client.config.Client.emoji.loading} Please wait while we are creating the roles!`
                );
                ...
                guildData.presenserole.gtav = guild.roles.cache.find(
                  (r) => r.name === "Grand Theft Auto V"
                )
                  ? guild.roles.cache.find(
                      (r) => r.name === "Grand Theft Auto V"
                    ).id
                  : null;
                guildData.presenserole.leagueoflegends = guild.roles.cache.find(
                  (r) => r.name === "League of Legends"
                )
                  ? guild.roles.cache.find(
                      (r) => r.name === "League of Legends"
                    ).id
                  : null;
                guildData.presenserole.roblox = guild.roles.cache.find(
                  (r) => r.name === "Roblox"
                )
                  ? guild.roles.cache.find((r) => r.name === "Roblox").id
                  : null;
                guildData.presenserole.minecraft = guild.roles.cache.find(
                  (r) => r.name === "Minecraft"
                )
                  ? guild.roles.cache.find((r) => r.name === "Minecraft").id
                  : null;
                guildData.presenserole.fortnite = guild.roles.cache.find(
                  (r) => r.name === "Fortnite"
                )
                  ? guild.roles.cache.find((r) => r.name === "Fortnite").id
                  : null;
                guildData.presenserole.valorant = guild.roles.cache.find(
                  (r) => r.name === "Valorant"
                )
                  ? guild.roles.cache.find((r) => r.name === "Valorant").id
                  : null;
                guildData.presenserole.spotify = guild.roles.cache.find(
                  (r) => r.name === "Spotify"
                )
                  ? guild.roles.cache.find((r) => r.name === "Spotify").id
                  : null;
                guildData.presenserole.netflix = guild.roles.cache.find(
                  (r) => r.name === "Netflix"
                )
                  ? guild.roles.cache.find((r) => r.name === "Netflix").id
                  : null;
                guildData.presenserole.twitch = guild.roles.cache.find(
                  (r) => r.name === "Twitch"
                )
                  ? guild.roles.cache.find((r) => r.name === "Twitch").id
                  : null;
                guildData.presenserole.vscode = guild.roles.cache.find(
                  (r) => r.name === "Visual Studio Code"
                )
                  ? guild.roles.cache.find(
                      (r) => r.name === "Visual Studio Code"
                    ).id
                  : null;
                await this.client.database.guildData.presenserole(
                  message?.guild.id,
                  guildData.presenserole
                );
                await nunumsg.edit({
                  content: `${this.client.config.Client.emoji.tick} Successfully enabled presence role!`,
                  components: [],
                });
                await this.client.sleep(1000);
                await nunumsg.delete().catch(() => {});
    ...
    let checkuser = await this.client.database.welcomeUserData.get(
      message?.member.id
    );
    if (checkuser.premium && checkuser.premiumCount > 0) {
      embed.setDescription(
        `${this.client.config.Client.emoji.info} You have premium! Would you like to activate the premium?`
      );
      let compos = [
        {
          type: 1,
    ...
    if (!antiNukeData.enabled) {
      embed.setDescription(
        `${this.client.config.Client.emoji.info} Would you like to enable Antinuke?`
      );
      let compos = [
        {
          type: 1,
      ...
      const collector15 = msg.createMessageComponentCollector({
        filter: filter15,
        time: 60000,
      });
      await new Promise((resolve) => {
    ...
    let successembed = this.client.util
      .embed()
      .setDescription(
        `${
          this.client.config.Client.emoji.Exclamation
        } Setup Wizard has been completed!\n\n${
          this.client.config.Client.emoji.add
        } Prefix: \`${guildData.prefix}\`\n${enabledaraay
          .map((x) => `${this.client.config.Client.emoji.tick} ${x}`)
          .join("\n")}\n${disabledaraay
          .map((x) => `${this.client.config.Client.emoji.cross} ${x}`)
          .join("\n")}`
      )
      .setColor(this.client.PrimaryColor)
      .setFooter({
        text: `Setup Wizard | ${message?.guild.name}`,
