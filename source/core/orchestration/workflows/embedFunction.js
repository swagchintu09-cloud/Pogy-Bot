...
module.exports = class EmbedFunction {
  ...
  async buildNormal(message, args, slash = false) {
    if (args === "create") {
      let check = await this.client.database.welcomeUserData.get(
      ...
      const msg = await message?.reply({
        content:
          "What would you like to set the embed name? (Type `cancel` to cancel)",
        fetchReply: true,
      });
    ...
    if (args == "show" || args == "list") {
      ...
      if (!data.message.length)
        return await message?.reply({
          content: "You Don't Have Any Presets Saved!",
        });
    ...
    if (args === "edit") {
      let data = await this.client.database.welcomeUserData.get(
        message?.member.user.id
      );
      if (!data.message.length)
        return await message?.reply({
      ...
      const embedValue = userData.message?.find(
        (g) => g.id === selected.values[0]
      );
  ...
  async body(message, data, embedData, i, embed, slash = false) {
    ...
    const descriptionData =
      modalSubmitInteraction.fields.getTextInputValue("description") == ""
        ? null
        : modalSubmitInteraction.fields.getTextInputValue("description");
    const dataDescription = await this.client.util.replace(
      descriptionData,
      message
    );
  ...
  async image(message, data, embedData, i, embed, slash = false) {
    ...
    const thumbnailData =
      modalSubmitInteraction.fields.getTextInputValue("thumbnail") === ""
        ? null
        : modalSubmitInteraction.fields.getTextInputValue("thumbnail");
    const dataThumbnail = await this.client.util.replaceIcon(
      thumbnailData,
      message
    );
  ...
  async misc(message, data, embedData, i, embed, slash = false) {
    ...
    const authorIconData =
      modalSubmitInteraction.fields.getTextInputValue("authorIcon") === ""
        ? null
        : modalSubmitInteraction.fields.getTextInputValue("authorIcon");
    const dataAuthorIcon = await this.client.util.replaceIcon(
      authorIconData,
      message
    );
    ...
    const footerIconData =
      modalSubmitInteraction.fields.getTextInputValue("footerIcon") === ""
        ? null
        : modalSubmitInteraction.fields.getTextInputValue("footerIcon");
    const dataFooterIcon = await this.client.util.replaceIcon(
      footerIconData,
      message
    );
